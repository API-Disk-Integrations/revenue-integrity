...
- Base URL is `https://revenueintegrity-api.com`. A client generated from an earlier copy of this contract points elsewhere and must be regenerated.
...
- Plans published. Billed per reconciliation.

| Plan | Per month | Included reconciliations | Additional | Overage cap |
| --- | --- | --- | --- | --- |
| Developer | $499 | 50 | $12.00 each | $1,000 |
| Growth | $1,499 | 250 | $8.00 each | $3,000 |
| Scale | $4,999 | 1,200 | $5.00 each | $10,000 |
| Enterprise | $10,000 | 4,000 | $3.00 each | $20,000 |
...
- `POST /v1/demo/reconcile` summary and description revised.
- `GET /v1/sandbox/example` summary and description revised.
...
- First published contract, 8 operations:
  ...
  - `POST /v1/reconciliations` — Reconcile one billing period
  - `POST /v1/demo/reconcile` (no API key required)
  - `GET /v1/sandbox/example` (no API key required)
...
- Free sandbox keys allow 5 reconciliations per period, no card required.
